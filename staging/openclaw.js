...
// include: /tmp/tmp1mfp3165.js
  ...
  (() => {
    ...
    async function loadPackage(metadata) {
      ...
      var PACKAGE_NAME = '/home/testuser/projects/.claude/worktrees/feat-game-loading-screen/Build_Release/openclaw.data';
    ...
    for (var file of metadata['files']) {
      ...
      async function processPackageData(arrayBuffer) {
        ...
        Module['removeRunDependency'](`fp ${name}`);
          ...
          Module['removeRunDependency']('datafile_/home/testuser/projects/.claude/worktrees/feat-game-loading-screen/Build_Release/openclaw.data');
      }
      Module['addRunDependency']('datafile_/home/testuser/projects/.claude/worktrees/feat-game-loading-screen/Build_Release/openclaw.data');
...
// end include: /tmp/tmp1mfp3165.js
...
function findWasmBinary() {
  return locateFile('openclaw.wasm');
}
...
/** @type {function(string, boolean=, number=)} */
  ...
  var _emscripten_wget_data = function(url, pbuffer, pnum, perror) {
    let innerFunc = async  () => {
...
var ASM_CONSTS = {
  725216: ($0) => { try { const name = UTF8ToString($0); let wavFileName = 'sounds/menu/CLICK.WAV'; if (name.includes('SELECT.WAV') || name.includes('SELECT_MENU_ITEM')) { wavFileName = 'sounds/menu/SELECT.WAV'; } else if (name.includes('CLICK.WAV') || name.includes('CHANGE_MENU_ITEM')) { wavFileName = 'sounds/menu/CLICK.WAV'; } else if (name.includes('MENUBED.WAV') || name.includes('MENUMUSIC')) { wavFileName = 'sounds/menu/MENUBED.WAV'; } fetch(wavFileName) .then(function(response) { if (!response.ok) { throw new Error('Failed to load WAV file: ' + response.status + ' ' + response.statusText); } return response.arrayBuffer(); }) .then(function(arrayBuffer) { const audioContext = window.audioContext; return audioContext.decodeAudioData(arrayBuffer); }) .then(function(audioBuffer) { window.soundBuffers = window.soundBuffers || new Map(); window.soundBuffers.set(name, audioBuffer); }) .catch(function(error) { console.error('Error loading WAV file:', error); window.soundBuffers = window.soundBuffers || new Map(); window.soundBuffers.set(name, { type: 'oscillator', frequency: 800, duration: 0.5 }); }); return true; } catch (e) { console.error('Error loading sound:', e); return false; } },  
 726402: ($0, $1) => { try { const name = UTF8ToString($0); const volume = $1; const soundBuffers = window.soundBuffers; if (!soundBuffers || !soundBuffers.has(name)) { return false; } const soundData = soundBuffers.get(name); const audioContext = window.audioContext; if (soundData.type === 'oscillator') { const oscillator = audioContext.createOscillator(); const gainNode = audioContext.createGain(); oscillator.frequency.setValueAtTime(soundData.frequency, audioContext.currentTime); oscillator.type = 'sine'; gainNode.gain.setValueAtTime(volume * window.soundVolume, audioContext.currentTime); gainNode.gain.exponentialRampToValueAtTime(0.01, audioContext.currentTime + soundData.duration); oscillator.connect(gainNode); gainNode.connect(audioContext.destination); oscillator.start(); oscillator.stop(audioContext.currentTime + soundData.duration); return true; } else { const source = audioContext.createBufferSource(); const gainNode = audioContext.createGain(); source.buffer = soundData; source.loop = false; gainNode.gain.value = volume * window.soundVolume; source.connect(gainNode); gainNode.connect(audioContext.destination); source.start(); return true; } } catch (e) { console.error('Error playing sound:', e); return false; } },  
 727625: ($0) => { if (window.activeSources && window.activeSources.has(UTF8ToString($0))) { const name = UTF8ToString($0); try { window.activeSources.get(name).stop(); window.activeSources.delete(name); } catch(e) { } } if (window.audioWorkletNode) { window.audioWorkletNode.port.postMessage({ type: 'stopSound', name: UTF8ToString($0) }); } },  
 727953: () => { if (window.activeSources) { window.activeSources.forEach((source, path) => { try { source.stop(); } catch(e) { } }); window.activeSources.clear(); } if (window.audioWorkletNode) { window.audioWorkletNode.port.postMessage({ type: 'stopAllSounds' }); } },  
 728208: ($0, $1, $2) => { if (window.audioWorkletNode) { window.audioWorkletNode.port.postMessage({ type: 'loadMusic', name: UTF8ToString($0), data: new Uint8Array(HEAPU8.buffer, $1, $2) }); } },  
 728379: ($0, $1, $2) => { if (window.audioWorkletNode) { window.audioWorkletNode.port.postMessage({ type: 'playMusic', name: UTF8ToString($0), looping: $1, volume: $2 }); } },  
 728530: () => { if (window.musicSource) { window.musicSource.stop(); window.musicSource = null; } if (window.musicGainNode) { window.musicGainNode = null; } if (window.stopLevelMidi) { window.stopLevelMidi(); } if (window.audioWorkletNode) { window.audioWorkletNode.port.postMessage({ type: 'stopMusic' }); } },  
 728827: () => { if (window.audioWorkletNode) { window.audioWorkletNode.port.postMessage({ type: 'pauseMusic' }); } },  
 728930: () => { if (window.audioWorkletNode) { window.audioWorkletNode.port.postMessage({ type: 'resumeMusic' }); } },  
 729034: ($0, $1, $2, $3, $4) => { try { const dataPtr = $0; const dataSize = $1; const volume = $2; const loops = $3; const originalPath = UTF8ToString($4); const audioData = new Uint8Array(dataSize); audioData.set(HEAPU8.subarray(dataPtr, dataPtr + dataSize)); const arrayBuffer = audioData.buffer; const header = String.fromCharCode.apply(null, audioData.slice(0, 4)); if (header !== 'RIFF') { console.error('[Audio] Invalid WAV header:', header); return false; } const audioContext = window.audioContext; audioContext.decodeAudioData(arrayBuffer.slice(0)) .then(function(audioBuffer) { window.soundBuffers = window.soundBuffers || new Map(); window.soundBuffers.set(originalPath, audioBuffer); const source = audioContext.createBufferSource(); const gainNode = audioContext.createGain(); source.buffer = audioBuffer; source.loop = (loops === -1); let volumeMultiplier = window.soundVolume; let isMusic = false; if (originalPath.includes('MENUBED.WAV') || originalPath.includes('MENUMUSIC')) { volumeMultiplier = window.musicVolume; isMusic = true; } let finalVolume = volume * volumeMultiplier; if (isMusic && !window.musicEnabled) { finalVolume = 0; } finalVolume = Math.max(0.0, Math.min(1.0, finalVolume)); gainNode.gain.value = finalVolume; if (isMusic) { if (window.musicSource) { try { window.musicSource.stop(); } catch(e) {} } window.musicGainNode = gainNode; window.musicSource = source; window.musicVolume = volumeMultiplier; } window.activeSources = window.activeSources || new Map(); const isLooping = (loops === -1); if (isLooping && !isMusic) { if (window.activeSources.has(originalPath)) { try { window.activeSources.get(originalPath).stop(); } catch(e) {} } window.activeSources.set(originalPath, source); } source.onended = function() { if (window.activeSources && window.activeSources.get(originalPath) === source) { window.activeSources.delete(originalPath); } }; source.connect(gainNode); gainNode.connect(audioContext.destination); source.start(); }) .catch(function(error) { console.error('[Audio] Failed to decode:', originalPath, error); }); return true; } catch (e) { console.error('[Audio] Exception:', e); return false; } },  
 731156: ($0) => { window.soundVolume = $0; if (window.audioWorkletNode) { window.audioWorkletNode.port.postMessage({ type: 'setSoundVolume', volume: $0 }); } },  
 731300: ($0) => { window.musicEnabled = $0; if (!window.musicEnabled) { if (window.musicGainNode) { window.musicGainNode.gain.value = 0; } } else { if (window.musicGainNode) { window.musicGainNode.gain.value = window.musicVolume; } } if (window.setLevelMidiVolume) { window.setLevelMidiVolume(); } if (window.audioWorkletNode) { window.audioWorkletNode.port.postMessage({ type: 'setMusicEnabled', enabled: $0 }); } },  
 731701: ($0) => { window.musicVolume = $0; if (window.musicGainNode && window.musicEnabled) { window.musicGainNode.gain.value = $0; } if (window.setLevelMidiVolume) { window.setLevelMidiVolume(); } if (window.audioWorkletNode) { window.audioWorkletNode.port.postMessage({ type: 'setMusicVolume', volume: $0 }); } },  
 732000: () => { try { if (!window.audioContext) { window.AudioContext = window.AudioContext || window.webkitAudioContext; if (!window.AudioContext) { console.error('Web Audio API not supported'); return false; } window.audioContext = new AudioContext(); } if (window.audioContext.state === 'suspended') { window.audioContext.resume(); } window.soundVolume = 1.0; window.musicVolume = 1.0; window.musicEnabled = true; window.soundEnabled = true; return true; } catch (e) { console.error('Error initializing AudioContext:', e); return false; } },  
 732530: () => { try { if (!window.audioContext || !window.audioContext.audioWorklet) { console.error('AudioWorklet not supported in this browser'); return false; } const audioWorkletScript = 'class GameAudioProcessor extends AudioWorkletProcessor {' + '    constructor() {' + '        super();' + '        this.soundVolume = 1.0;' + '        this.musicVolume = 1.0;' + '        this.port.onmessage = (event) => { this.handleMessage(event.data); };' + '    }' + '    handleMessage(data) {' + '        switch (data.type) {' + '            case "setSoundVolume": this.soundVolume = data.volume; break;' + '            case "setMusicVolume": this.musicVolume = data.volume; break;' + '        }' + '    }' + '    process(inputs, outputs, parameters) {' + '        // Simple audio processing - just pass through with volume control' + '        const output = outputs[0];' + '        if (output && output.length > 0) {' + '            for (let channel = 0; channel < output.length; channel++) {' + '                const outputChannel = output[channel];' + '                if (outputChannel) {' + '                    for (let i = 0; i < outputChannel.length; i++) {' + '                        outputChannel[i] = 0; // Clear output' + '                    }' + '                }' + '            }' + '        }' + '        return true;' + '    }' + '}' + 'registerProcessor("game-audio-processor", GameAudioProcessor);'; const blob = new Blob([audioWorkletScript], { type: 'application/javascript' }); const url = URL.createObjectURL(blob); return new Promise((resolve, reject) => { window.audioContext.audioWorklet.addModule(url) .then(() => { try { window.audioWorkletNode = new AudioWorkletNode(window.audioContext, 'game-audio-processor'); window.audioWorkletNode.connect(window.audioContext.destination); resolve(true); } catch (e) { console.error('Error creating AudioWorkletNode:', e); reject(e); } }) .catch(error => { console.error('Error loading AudioWorklet module:', error); reject(error); }); }).then(() => true).catch(() => false); } catch (e) { console.error('Error setting up AudioWorklet:', e); return false; } },  
 734643: ($0) => { if (window.audioWorkletNode) { window.audioWorkletNode.port.postMessage(JSON.parse(UTF8ToString($0))); } },  
 734752: () => { console.log('[Debug] m_videoPlatform is NULL!'); },  
 734805: () => { var canvas = document.getElementById('canvas'); return canvas ? canvas.width : 640; },  
 734893: () => { var canvas = document.getElementById('canvas'); return canvas ? canvas.height : 480; },  
 734982: ($0, $1) => { if (typeof triggerHaptic === 'function') { triggerHaptic($0, UTF8ToString($1)); } },  
 735068: ($0, $1, $2, $3) => { if (typeof triggerHapticCustom === 'function') { triggerHapticCustom($0, $1, $2, $3); } },  
 735160: ($0) => { if (typeof setHapticEnabled === 'function') { setHapticEnabled($0); } },  
 735234: ($0) => { try { localStorage.setItem('claw-web:saves', UTF8ToString($0)); return 1; } catch (e) { console.error('[SaveBridge] Save error:', e); return 0; } },  
 735384: () => { try { var jsonStr = localStorage.getItem('claw-web:saves'); if (!jsonStr) return 0; var lengthBytes = lengthBytesUTF8(jsonStr) + 1; var ptr = _malloc(lengthBytes); stringToUTF8(jsonStr, ptr, lengthBytes); return ptr; } catch (e) { console.warn('[SaveBridge] Load error:', e); return 0; } },  
 735676: () => { try { return localStorage.getItem('claw-web:saves') ? 1 : 0; } catch (e) { return 0; } },  
 735767: () => { try { localStorage.removeItem('claw-web:saves'); return 1; } catch (e) { console.error('[SaveBridge] Delete error:', e); return 0; } },  
 735904: () => { if (typeof window.webglBridge !== 'undefined' && window.webglBridge.cleanup) { window.webglBridge.cleanup(); } },  
 736019: ($0, $1, $2, $3) => { if (typeof window.webglBridge !== 'undefined' && window.webglBridge.setViewport) { window.webglBridge.setViewport($0, $1, $2, $3); } },  
 736156: ($0, $1, $2, $3) => { if (typeof window.webglBridge !== 'undefined' && window.webglBridge.clear) { window.webglBridge.clear($0, $1, $2, $3); } },  
 736281: () => { if (typeof window.webglBridge !== 'undefined' && window.webglBridge.present) { window.webglBridge.present(); } },  
 736396: () => { if (typeof window.webglBridge !== 'undefined' && window.webglBridge.initialize) { return window.webglBridge.initialize() ? 1 : 0; } return 0; },  
 736542: () => { if (typeof window.webglBridge !== 'undefined' && window.webglBridge.createShaders) { return window.webglBridge.createShaders() ? 1 : 0; } return 0; },  
 736694: () => { if (typeof window.webglBridge !== 'undefined' && window.webglBridge.createBuffers) { return window.webglBridge.createBuffers() ? 1 : 0; } return 0; },  
 736846: ($0, $1, $2, $3, $4, $5, $6, $7) => { if (typeof window.webglBridge !== 'undefined' && window.webglBridge.renderQuad) { window.webglBridge.renderQuad($0, $1, $2, $3, $4, $5, $6, $7); } },  
 736997: () => { if (!window.textureBridge) { window.textureBridge = new TextureBridge(); } window.textureBridge.initialize(); },  
 737111: () => { if (window.textureBridge) { window.textureBridge.shutdown(); delete window.textureBridge; window.textureBridge = null; } },  
 737236: ($0, $1) => { if (window.textureBridge) { return window.textureBridge.loadTexture(UTF8ToString($0), $1) ? 1 : 0; } return 0; },  
 737351: ($0) => { return window.textureBridge ? window.textureBridge.getTextureWidth($0) : 0; },  
 737431: ($0) => { return window.textureBridge ? window.textureBridge.getTextureHeight($0) : 0; },  
 737512: ($0) => { return window.textureBridge ? window.textureBridge.getTextureFormat($0) : 0; },  
 737593: ($0) => { return window.textureBridge ? window.textureBridge.hasTransparency($0) : 0; },  
 737673: ($0) => { if (window.textureBridge) { window.textureBridge.unloadTexture($0); } },  
 737747: ($0, $1) => { if (window.playLevelMidiBuffer) { var bytes = HEAPU8.slice($0, $0 + $1); window.playLevelMidiBuffer(bytes); } },  
 737861: () => { window.exportSaveData(); },  
 737890: () => { window.importSaveData(); },  
 737919: () => { window.toggleFullscreen(); },  
 737950: () => { window.toggleAspectRatio(); },  
 737982: () => { return document.fullscreenElement ? 1 : 0; },  
 738029: () => { return document.fullscreenElement ? 0 : 1; },  
 738076: () => { return window.forceOriginalAspect ? 0 : 1; },  
 738123: () => { return window.forceOriginalAspect ? 1 : 0; },  
 738170: () => { var iOS = /iPad|iPhone|iPod/.test(navigator.userAgent) || (navigator.platform === "MacIntel" && navigator.maxTouchPoints > 1); if (iOS) return 0; return document.fullscreenElement ? 1 : 0; },  
 738363: () => { var iOS = /iPad|iPhone|iPod/.test(navigator.userAgent) || (navigator.platform === "MacIntel" && navigator.maxTouchPoints > 1); if (iOS) return 0; return document.fullscreenElement ? 0 : 1; },  
 738556: () => { return window.forceOriginalAspect ? 0 : 1; },  
 738603: () => { return window.forceOriginalAspect ? 1 : 0; },  
 738650: ($0) => { console.log('[Gamepad C++] Controller ' + $0 + ' connected'); },  
 738716: ($0) => { console.log('[Gamepad C++] Controller ' + $0 + ' disconnected'); },  
 738785: () => { return window.devicePixelRatio ? window.devicePixelRatio : 1.0; },  
 738853: ($0) => { var str = UTF8ToString($0) + '\n\n' + 'Abort/Retry/Ignore/AlwaysIgnore? [ariA] :'; var reply = window.prompt(str, "i"); if (reply === null) { reply = "i"; } return reply.length === 1 ? reply.charCodeAt(0) : -1; },  
 739068: () => { if (typeof(AudioContext) !== 'undefined') { return true; } else if (typeof(webkitAudioContext) !== 'undefined') { return true; } return false; },  
 739215: () => { if ((typeof(navigator.mediaDevices) !== 'undefined') && (typeof(navigator.mediaDevices.getUserMedia) !== 'undefined')) { return true; } else if (typeof(navigator.webkitGetUserMedia) !== 'undefined') { return true; } return false; },  
 739449: ($0) => { if(typeof(Module['SDL2']) === 'undefined') { Module['SDL2'] = {}; } var SDL2 = Module['SDL2']; if (!$0) { SDL2.audio = {}; } else { SDL2.capture = {}; } if (!SDL2.audioContext) { if (typeof(AudioContext) !== 'undefined') { SDL2.audioContext = new AudioContext(); } else if (typeof(webkitAudioContext) !== 'undefined') { SDL2.audioContext = new webkitAudioContext(); } if (SDL2.audioContext) { if ((typeof navigator.userActivation) === 'undefined') { autoResumeAudioContext(SDL2.audioContext); } } } return SDL2.audioContext === undefined ? -1 : 0; },  
 740001: () => { var SDL2 = Module['SDL2']; return SDL2.audioContext.sampleRate; },  
 740069: ($0, $1, $2, $3) => { var SDL2 = Module['SDL2']; var have_microphone = function(stream) { if (SDL2.capture.silenceTimer !== undefined) { clearInterval(SDL2.capture.silenceTimer); SDL2.capture.silenceTimer = undefined; SDL2.capture.silenceBuffer = undefined } SDL2.capture.mediaStreamNode = SDL2.audioContext.createMediaStreamSource(stream); SDL2.capture.scriptProcessorNode = SDL2.audioContext.createScriptProcessor($1, $0, 1); SDL2.capture.scriptProcessorNode.onaudioprocess = function(audioProcessingEvent) { if ((SDL2 === undefined) || (SDL2.capture === undefined)) { return; } audioProcessingEvent.outputBuffer.getChannelData(0).fill(0.0); SDL2.capture.currentCaptureBuffer = audioProcessingEvent.inputBuffer; dynCall('vp', $2, [$3]); }; SDL2.capture.mediaStreamNode.connect(SDL2.capture.scriptProcessorNode); SDL2.capture.scriptProcessorNode.connect(SDL2.audioContext.destination); SDL2.capture.stream = stream; }; var no_microphone = function(error) { }; SDL2.capture.silenceBuffer = SDL2.audioContext.createBuffer($0, $1, SDL2.audioContext.sampleRate); SDL2.capture.silenceBuffer.getChannelData(0).fill(0.0); var silence_callback = function() { SDL2.capture.currentCaptureBuffer = SDL2.capture.silenceBuffer; dynCall('vp', $2, [$3]); }; SDL2.capture.silenceTimer = setInterval(silence_callback, ($1 / SDL2.audioContext.sampleRate) * 1000); if ((navigator.mediaDevices !== undefined) && (navigator.mediaDevices.getUserMedia !== undefined)) { navigator.mediaDevices.getUserMedia({ audio: true, video: false }).then(have_microphone).catch(no_microphone); } else if (navigator.webkitGetUserMedia !== undefined) { navigator.webkitGetUserMedia({ audio: true, video: false }, have_microphone, no_microphone); } },  
 741762: ($0, $1, $2, $3) => { var SDL2 = Module['SDL2']; SDL2.audio.scriptProcessorNode = SDL2.audioContext['createScriptProcessor']($1, 0, $0); SDL2.audio.scriptProcessorNode['onaudioprocess'] = function (e) { if ((SDL2 === undefined) || (SDL2.audio === undefined)) { return; } if (SDL2.audio.silenceTimer !== undefined) { clearInterval(SDL2.audio.silenceTimer); SDL2.audio.silenceTimer = undefined; SDL2.audio.silenceBuffer = undefined; } SDL2.audio.currentOutputBuffer = e['outputBuffer']; dynCall('vp', $2, [$3]); }; SDL2.audio.scriptProcessorNode['connect'](SDL2.audioContext['destination']); if (SDL2.audioContext.state === 'suspended') { SDL2.audio.silenceBuffer = SDL2.audioContext.createBuffer($0, $1, SDL2.audioContext.sampleRate); SDL2.audio.silenceBuffer.getChannelData(0).fill(0.0); var silence_callback = function() { if ((typeof navigator.userActivation) !== 'undefined') { if (navigator.userActivation.hasBeenActive) { SDL2.audioContext.resume(); } } SDL2.audio.currentOutputBuffer = SDL2.audio.silenceBuffer; dynCall('vp', $2, [$3]); SDL2.audio.currentOutputBuffer = undefined; }; SDL2.audio.silenceTimer = setInterval(silence_callback, ($1 / SDL2.audioContext.sampleRate) * 1000); } },  
 ...
 743542: ($0, $1) => { var SDL2 = Module['SDL2']; var buf = $0 >>> 2; var numChannels = SDL2.audio.currentOutputBuffer['numberOfChannels']; for (var c = 0; c < numChannels; ++c) { var channelData = SDL2.audio.currentOutputBuffer['getChannelData'](c); if (channelData.length != $1) { throw 'Web Audio output buffer length mismatch! Destination size: ' + channelData.length + ' samples vs expected ' + $1 + ' samples!'; } for (var j = 0; j < $1; ++j) { channelData[j] = HEAPF32[buf + (j*numChannels + c)]; } } },  
 744031: ($0) => { var SDL2 = Module['SDL2']; if ($0) { if (SDL2.capture.silenceTimer !== undefined) { clearInterval(SDL2.capture.silenceTimer); } if (SDL2.capture.stream !== undefined) { var tracks = SDL2.capture.stream.getAudioTracks(); for (var i = 0; i < tracks.length; i++) { SDL2.capture.stream.removeTrack(tracks[i]); } } if (SDL2.capture.scriptProcessorNode !== undefined) { SDL2.capture.scriptProcessorNode.onaudioprocess = function(audioProcessingEvent) {}; SDL2.capture.scriptProcessorNode.disconnect(); } if (SDL2.capture.mediaStreamNode !== undefined) { SDL2.capture.mediaStreamNode.disconnect(); } SDL2.capture = undefined; } else { if (SDL2.audio.scriptProcessorNode != undefined) { SDL2.audio.scriptProcessorNode.disconnect(); } if (SDL2.audio.silenceTimer !== undefined) { clearInterval(SDL2.audio.silenceTimer); } SDL2.audio = undefined; } if ((SDL2.audioContext !== undefined) && (SDL2.audio === undefined) && (SDL2.capture === undefined)) { SDL2.audioContext.close(); SDL2.audioContext = undefined; } },  
 745037: ($0, $1, $2) => { var w = $0; var h = $1; var pixels = $2; if (!Module['SDL2']) Module['SDL2'] = {}; var SDL2 = Module['SDL2']; if (SDL2.ctxCanvas !== Module['canvas']) { SDL2.ctx = Browser.createContext(Module['canvas'], false, true); SDL2.ctxCanvas = Module['canvas']; } if (SDL2.w !== w || SDL2.h !== h || SDL2.imageCtx !== SDL2.ctx) { SDL2.image = SDL2.ctx.createImageData(w, h); SDL2.w = w; SDL2.h = h; SDL2.imageCtx = SDL2.ctx; } var data = SDL2.image.data; var src = pixels / 4; var dst = 0; var num; if (typeof CanvasPixelArray !== 'undefined' && data instanceof CanvasPixelArray) { num = data.length; while (dst < num) { var val = HEAP32[src]; data[dst ] = val & 0xff; data[dst+1] = (val >> 8) & 0xff; data[dst+2] = (val >> 16) & 0xff; data[dst+3] = 0xff; src++; dst += 4; } } else { if (SDL2.data32Data !== data) { SDL2.data32 = new Int32Array(data.buffer); SDL2.data8 = new Uint8Array(data.buffer); SDL2.data32Data = data; } var data32 = SDL2.data32; num = data32.length; data32.set(HEAP32.subarray(src, src + num)); var data8 = SDL2.data8; var i = 3; var j = i + 4*num; if (num % 8 == 0) { while (i < j) { data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; } } else { while (i < j) { data8[i] = 0xff; i = i + 4 | 0; } } } SDL2.ctx.putImageData(SDL2.image, 0, 0); },  
 746503: ($0, $1, $2, $3, $4) => { var w = $0; var h = $1; var hot_x = $2; var hot_y = $3; var pixels = $4; var canvas = document.createElement("canvas"); canvas.width = w; canvas.height = h; var ctx = canvas.getContext("2d"); var image = ctx.createImageData(w, h); var data = image.data; var src = pixels / 4; var dst = 0; var num; if (typeof CanvasPixelArray !== 'undefined' && data instanceof CanvasPixelArray) { num = data.length; while (dst < num) { var val = HEAP32[src]; data[dst ] = val & 0xff; data[dst+1] = (val >> 8) & 0xff; data[dst+2] = (val >> 16) & 0xff; data[dst+3] = (val >> 24) & 0xff; src++; dst += 4; } } else { var data32 = new Int32Array(data.buffer); num = data32.length; data32.set(HEAP32.subarray(src, src + num)); } ctx.putImageData(image, 0, 0); var url = hot_x === 0 && hot_y === 0 ? "url(" + canvas.toDataURL() + "), auto" : "url(" + canvas.toDataURL() + ") " + hot_x + " " + hot_y + ", auto"; var urlBuf = _malloc(url.length + 1); stringToUTF8(url, urlBuf, url.length + 1); return urlBuf; },  
 747491: ($0) => { if (Module['canvas']) { Module['canvas'].style['cursor'] = UTF8ToString($0); } },  
 747574: () => { if (Module['canvas']) { Module['canvas'].style['cursor'] = 'none'; } },  
 747643: () => { return window.innerWidth; },  
 747673: () => { return window.innerHeight; }
};
function js_CreatePlayer() { if (!Module.genericVideoPlayers) Module.genericVideoPlayers = {}; var id = Object.keys(Module.genericVideoPlayers).length + 1; Module.genericVideoPlayers[id] = { video : document.createElement('video'), ready : false }; return id; }
function js_DestroyPlayer(id) { if (Module.genericVideoPlayers && Module.genericVideoPlayers[id]) { var player = Module.genericVideoPlayers[id]; if (player.keyHandler) { document.removeEventListener('keydown', player.keyHandler, true); player.keyHandler = null; } if (player.video && player.video.parentNode) { player.video.parentNode.removeChild(player.video); } delete Module.genericVideoPlayers[id]; } }
function js_LoadVideo(id,url) { var player = Module.genericVideoPlayers[id]; if (player) { player.ended = false; player.error = false; player.video.crossOrigin = "anonymous"; player.video.src = UTF8ToString(url); player.video.onended = function() { player.ended = true; if (player.keyHandler) { document.removeEventListener('keydown', player.keyHandler, true); player.keyHandler = null; } if (player.video.parentNode) { player.video.parentNode.removeChild(player.video); } }; player.video.onerror = function() { player.error = true; if (player.keyHandler) { document.removeEventListener('keydown', player.keyHandler, true); player.keyHandler = null; } if (player.video.parentNode) { player.video.parentNode.removeChild(player.video); } }; } }
function js_StopVideo(id) { var player = Module.genericVideoPlayers ? Module.genericVideoPlayers[id] : null; if (!player || !player.video) return; try { player.video.pause(); player.video.currentTime = 0; } catch (e) { } player.ended = true; if (player.keyHandler) { document.removeEventListener('keydown', player.keyHandler, true); player.keyHandler = null; } if (player.gamepadPollId) { cancelAnimationFrame(player.gamepadPollId); player.gamepadPollId = null; } if (player.video.parentNode) { player.video.parentNode.removeChild(player.video); } if (Module['canvas'] && Module['canvas'].focus) { Module['canvas'].focus(); } else { var c = document.querySelector('canvas'); if (c && c.focus) c.focus(); } }
function js_GetVideoState(id) { var player = Module.genericVideoPlayers[id]; if (!player || !player.video) return 0; var state = 0; if (!player.video.paused) state |= 1; if (player.ended) state |= 2; if (player.ready) state |= 4; if (player.error) state |= 8; return state; }
function js_PlayVideo(id) { var player = Module.genericVideoPlayers[id]; if (player && player.video) { if (player.keyHandler) { document.removeEventListener('keydown', player.keyHandler, true); player.keyHandler = null; } if (player.gamepadPollId) { cancelAnimationFrame(player.gamepadPollId); player.gamepadPollId = null; } player.keyHandler = function(e) { if (e.key == 'Escape' || e.key == 'Enter') { e.preventDefault(); e.stopPropagation(); js_StopVideo(id); } }; document.addEventListener('keydown', player.keyHandler, true); var lastButtonStates = {}; function pollGamepadForSkip() { if (player.ended) return; var gamepads = navigator.getGamepads(); for (var i = 0; i < 4; i++) { var gp = gamepads[i]; if (gp && gp.connected) { var skipButtons = [0, 9]; for (var j = 0; j < skipButtons.length; j++) { var btn = skipButtons[j]; if (gp.buttons[btn]) { var pressed = gp.buttons[btn].pressed; var key = i + '_' + btn; var wasPressed = lastButtonStates[key] || false; if (pressed && !wasPressed) { console.log('[Video] Gamepad button ' + btn + ' pressed - skipping video'); js_StopVideo(id); return; } lastButtonStates[key] = pressed; } } } } player.gamepadPollId = requestAnimationFrame(pollGamepadForSkip); } player.gamepadPollId = requestAnimationFrame(pollGamepadForSkip); var gameContainer = document.getElementById('gameContainer'); if (!gameContainer) { gameContainer = document.body; } player.video.style.position = 'absolute'; player.video.style.top = '0'; player.video.style.left = '0'; player.video.style.width = '100%'; player.video.style.height = '100%'; player.video.style.objectFit = 'contain'; player.video.style.zIndex = '10'; player.video.style.backgroundColor = '#000'; gameContainer.appendChild(player.video); player.video.play().catch(function(err) { player.error = true; }); } }
function js_PauseVideo(id) { var player = Module.genericVideoPlayers[id]; if (player && player.video) player.video.pause(); }

// Imports from the Wasm binary.
var _main,
  _free,
  _OnJSSaveDataImported,
  _malloc,
  _OnJSGamepadConnected,
  _OnJSGamepadDisconnected,
  _OnJSGamepadButtonDown,
  _OnJSGamepadButtonUp,
  _OnJSGamepadAxis,
  _OnJSGamepadActivity,
  _OnJSPointerDown,
  _OnJSPointerMove,
  _OnJSPointerUp,
...
function assignWasmExports(wasmExports) {
  ...
  _OnJSSaveDataImported = Module['_OnJSSaveDataImported'] = wasmExports['OnJSSaveDataImported'];
  _malloc = wasmExports['malloc'];
  _OnJSGamepadConnected = Module['_OnJSGamepadConnected'] = wasmExports['OnJSGamepadConnected'];
  _OnJSGamepadDisconnected = Module['_OnJSGamepadDisconnected'] = wasmExports['OnJSGamepadDisconnected'];
  _OnJSGamepadButtonDown = Module['_OnJSGamepadButtonDown'] = wasmExports['OnJSGamepadButtonDown'];
  _OnJSGamepadButtonUp = Module['_OnJSGamepadButtonUp'] = wasmExports['OnJSGamepadButtonUp'];
  _OnJSGamepadAxis = Module['_OnJSGamepadAxis'] = wasmExports['OnJSGamepadAxis'];
  _OnJSGamepadActivity = Module['_OnJSGamepadActivity'] = wasmExports['OnJSGamepadActivity'];
  _OnJSPointerDown = Module['_OnJSPointerDown'] = wasmExports['OnJSPointerDown'];
  _OnJSPointerMove = Module['_OnJSPointerMove'] = wasmExports['OnJSPointerMove'];
  ...
  _GetGameHeight = Module['_GetGameHeight'] = wasmExports['GetGameHeight'];
  _OnJSFullscreenChange = Module['_OnJSFullscreenChange'] = wasmExports['OnJSFullscreenChange'];
  ...
  dynCall_iiiiiiiiiiiiiiff = dynCalls['iiiiiiiiiiiiiiff'] = wasmExports['dynCall_iiiiiiiiiiiiiiff'];
  ...
  dynCall_iiiiiidiiff = dynCalls['iiiiiidiiff'] = wasmExports['dynCall_iiiiiidiiff'];
